...
SfDatagrid loads two different icons for denoting the `Ascending` and `Descending` sort direction state. However, SfDataGrid allows you to rotate the `DataGridStyle.GetHeaderSortIndicatorUp` icon animatedly for denoting the decending state by overriding the `DataGridStyle.GetHeaderSortIndicatorDown` method and returning `0`.
