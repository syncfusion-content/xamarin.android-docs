...
[VerticalOverScrollMode](https://help.syncfusion.com/cr/xamarin/sfdatagrid) property allows you to enable or disable the bouncing effect in the grid which has two modes `Bounce` and `None`. By Default, `SfDataGrid.VerticalOverScrollMode` value is `Bounce` which enables the bouncing effect. And `SfDataGrid.VerticalOverScrollMode. None` disables the bouncing effect.
{% highlight C# %}
